...
var h3 = document.getElementsByTagName('h3')[0],
  userUl = document.querySelectorAll('ul')[0];
...
var userNames = ['Mark', 'Daniele', 'Samuel', 'Cisco'];
...
console.log(form);
form[1].addEventListener('click', function(evt){
  evt.preventDefault();
  if(form[0].value){
    userNames.push(properCapString(form[0].value));
    populateUserList();
  }
  form[0].value = '';
});
  ...
  if(userNames.length){
    ...
    for(let i = 0; i < userNames.length; i++){
      ...
      span.addEventListener('click', function(evt){
        ...
        userNames.splice(this.parentElement.dataset.index, 1);
        populateUserList();
      });
